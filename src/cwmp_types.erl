...
get_xmlText(Content) ->
    XS = lists:filter(fun cwmp_lib:xmlText/1, Content),
...
parse_XS_Array(Mapper, #xmlElement{content = Content} = E,
    ...
    List = [Mapper(Elem, State) || Elem <- Content, cwmp_lib:xmlElement(Elem)],
    if
	length(List) == Size ->
	    List;
	true ->
...
parse_ArraySize_test() ->
    [
	 ...
	 Num = cwmp_types:parse_ArraySize(Value, Tag, Nss),
	 ?DBG(Num),
